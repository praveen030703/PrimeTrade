...
const Task = () => {
  ...
  useEffect(() => {
    ...
    if (editTask && tasks.length > 0) {
      const task = tasks.find((item) => (item._id || item.id) === (editTask._id || editTask.id))
      if (task) {
  ...
  const resetForm = () => {
    setTaskForm({ title: '', description: '', status: 'Pending', dueDate: '', image: null })
    setEditingId(null)
  ...
  const handleSubmit = async (event) => {
    ...
    try {
      ...
      let response
      if (editingId) {
        response = await fetch(`http://localhost:5000/api/auth/update-task/${editingId}`, {
          method: 'PUT',
          headers: {
            'Authorization': `Bearer ${token}`,
          },
        ...
        response = await fetch('http://localhost:5000/api/auth/create-task', {
          method: 'POST',
          headers: {
            'Authorization': `Bearer ${token}`,
          },
  ...
  const handleEdit = (taskId) => {
    const task = tasks.find((item) => item._id === taskId || item.id === taskId)
    if (!task) return
  ...
  return (
    <div className="space-y-8">
      <section className="card p-6">
        ...
        <form className="mt-6 grid gap-4 md:grid-cols-2 md:gap-6" onSubmit={handleSubmit}>
          {error && (
